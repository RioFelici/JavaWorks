...
public class EmployeeList {
    ...
    public void getData(String input) {
        ...
        for (int i = 0; i < data.length; i += 3) {
            ...
            float hours = Float.parseFloat(data[i+3]);
            if (i + 3 < data.length && data[i + 3].matches("\\d+(\\.\\d*)?")) {
                float wage = Float.parseFloat(data[i + 3]);
                employees.add(new HourlyEmployee(name, age, wage, hours));
            } else
...
/*        for (int i=0 ; i<length()-1 ; i++)
    ...
    public void sortByAge() {
        Collections.sort(employees, (e1, e2) -> Float.compare(e1.getAge(), e2.getAge()));
    }
}
